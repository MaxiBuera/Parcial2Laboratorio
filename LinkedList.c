...
/** \brief La función ll_filter recibirá una lista y una función criterio fn.
  	  	  ...
  	  	  La función ll_filter generará la nueva lista resultado, agregará a la misma
  	  	  los ítems correspondientes y la devolverá.
 *
 * \param  	lista
 * \param	funcion
 * \return 	(1) si ese ítem se debe agregar a la lista resultado
        ...
        if(pListaFiltrada != NULL)
        {
            for(i=0;i<ll_len(this);i++)
            {
                ...
                if(pElement != NULL)
                {
                    ...
                    if(retorno == 0)
                    {
                        ll_add(pListaFiltrada, pElement);
                    }
                }
            }
            pReturnLista = pListaFiltrada;
        }
    }
